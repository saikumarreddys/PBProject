...
df = spark.read.json("AlertParse1.txt")
df.createOrReplaceTempView("hashtags")


sqldf = spark.sql("SELECT Country,count(*) as Totalalerts \
                 FROM hashtags  \
                 group by Country, order by Totalalerts desc")
sqldf.show(150)

sqldf.toPandas().to_csv('Alertanalysis1.csv')
